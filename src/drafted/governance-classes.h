 ...
 *
 *   // network
 *   CArchimedNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.archimed.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CArchimedNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CArchimedNetwork(UniValue objIn)
    {
...
// // can be under: ArchimedNetwork
// //   -- signature requirements : Key1(User)
// class CArchimedNetworkVariable : public CGovernanceObject
// {
// private:
