...
const char * const BITCOIN_CONF_FILENAME = "archimed.conf";
const char * const BITCOIN_PID_FILENAME = "archimedd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "archimed" is a composite category enabling all Archimed-related debug output
            if(ptrCategory->count(string("archimed"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArchimedCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArchimedCore
    // Mac: ~/Library/Application Support/ArchimedCore
    // Unix: ~/.archimedcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArchimedCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArchimedCore";
#else
    // Unix
    return pathRet / ".archimedcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty archimed.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override archimed.conf
