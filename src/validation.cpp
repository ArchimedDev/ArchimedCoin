...
#if defined(NDEBUG)
# error "Archimed Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
      nSubsidyBase = 50400;
    } else if (nPrevHeight < 100 && nPrevHeight > 0) {
      nSubsidyBase = 1;
    } else {
      nSubsidyBase = 30;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight > 100){
      ...
      ret = (blockValue/100) * 90;
    } else if (nHeight > 77760) {
      ret = (blockValue/100) * 75;
    } else if (nHeight > 86400) {
      ret = (blockValue/100) * 90;
    } else if (nHeight > 95040) {
      ret = (blockValue/100) * 75;
    } else if (nHeight > 103680) {
      ret = (blockValue/100) * 90;
...
void ThreadScriptCheck() {
    RenameThread("archimed-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // ARCHIMED : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ARCHIMED): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ARCHIMED): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ARCHIMED : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ARCHIMED): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ARCHIMED): spork is off, skipping transaction locking checks\n");
    }
